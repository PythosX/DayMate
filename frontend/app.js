...
function renderAll(){
  ...
  const tasksToday=DATA.tasks.filter(x=>x.date===today()&&!x.done).length;
  $("todayCount").textContent=tasksToday;
  $("importantCount").textContent=DATA.important.length+DATA.tell_later.filter(x=>!x.done).length;
  ...
  const home=[...DATA.tasks.filter(x=>x.date===today()&&!x.done).slice(0,3),...DATA.important.slice(0,2),...DATA.tell_later.filter(x=>!x.done).slice(0,2)];
  $("homeItems").innerHTML=home.map(x=>x.person!==undefined?tellHTML(x):x.category==="deadline"||DATA.important.includes(x)?importantHTML(x):taskHTML(x)).join("")||empty("Nothing urgent. Your mind is clear.");
}
